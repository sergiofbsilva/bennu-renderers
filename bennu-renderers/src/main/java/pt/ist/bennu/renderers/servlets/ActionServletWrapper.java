 ...
 */
package pt.ist.bennu.renderers.servlets;
...
public class ActionServletWrapper extends ActionServlet {
    ...
    private static class ServletConfigWrapper implements ServletConfig {
        ...
        static {
            ...
            parameterMap.put("application", "RendererResources");
    ...
    protected ModuleConfig initModuleConfig(String prefix, String paths) throws ServletException {
        ...
        try {
            ...
            for (FenixFrameworkArtifact artifact : FenixFrameworkArtifact.fromName(ConfigurationManager.getProperty("app.name"))
                    .getArtifacts()) {
                try (InputStream stream = loader.getResourceAsStream(artifact.getName() + "/.messageResources")) {
                    if (stream != null) {
                        try (InputStreamReader reader = new InputStreamReader(stream, Charsets.UTF_8)) {
                            List<String> resources = CharStreams.readLines(reader);
                            for (String resource : resources) {
                                ...
                                if (resource.equals("BennuResources")) {
